  ...
  return (
    <>
      ...
      <WrapperComponent
        classes={{
          sectionClass:
            "banner-padding banner-section absolute-banner pb-0 ratio2_1",
          fluidClass: "container overflow-hidden absolute-bg",
        }}
        customCol={true}
      >
        {banners.map(
          (banner, i) =>
            banner?.status && (
              <div
                key={i}
                className={`${banners.length === 3 ? "col-md-4 col-sm-6" : banners.length === 2 ? "col-md-6" : "col-12"}`}
              >
      ...
      <Container className="no-arrow">
        <section className="category-style-1 section-t-space">
          <HomeCategorySidebar
            style="simple"
            sliderOptions={categorySlider5}
            slider={true}
            categoryIds={data?.categories_icon_list?.category_ids || []}
          />
        </section>
      </Container>
      ...
      {data?.product_list_2?.status && (
        <WrapperComponent
          classes={{ fluidClass: "container", row: "g-4" }}
          customCol={true}
        >
          <div
            className={
          ...
          >
            <div className="theme-tab">
              <div className="tab-content-cls">
                <HomeProduct
                  productIds={
                    data?.product_list_2?.products?.product_item?.product_ids ||
                    []
                  }
                  slider={true}
              ...
              {(data?.product_list_2?.products?.product_banner?.banner_1
                ?.status ||
                data?.product_list_2?.products?.product_banner?.banner_2
                  ?.status) && (
                <div className="banner-tools border-0 product_banner banner-section">
                  <div className="ratio2_1">
                    <Container className=" p-0">
                      <Row className="g-4">
                        {data?.product_list_2?.products?.product_banner
                          ?.banner_1?.status && (
                          <div
                            className={
                              data?.product_list_2?.products?.product_banner
            ...
            <Col xl="3" lg="4" md="12" className="mb-0 images">
              <div className="theme-card">
                <h5 className="title-border">
                  {data?.product_list_2?.right_panel?.title}
                </h5>
                <div className="offer-slider ">
                  <HomeProduct
                    productIds={
                      data?.product_list_2?.right_panel?.product_ids || []
                    }
                    style="horizontal"
                  />
                </div>
