...
const TrendingProduct = ({ productState }) => {
  ...
  const { data: productData, refetch: productRefetch } = useFetchQuery(
    [categoryId],
    () =>
      request({
        url: ProductAPI,
      ...
      <div className="offer-slider">
        {productData?.slice(0, 4)?.map((elem, i) => (
          <Media className="mb-2" key={i}>
            <ImageLink
              imgUrl={elem?.product_galleries[0]?.original_url}
              width={105}
              height={120}
            />
            <Media body>
              <RatingBox
                totalRating={
                  elem?.rating_count ?? productState?.product?.rating_count
                }
              />
              <Link href={`/product/${elem?.id || elem?.slug}`}>
                <h6>{elem?.name}</h6>
              </Link>
              <h4>
