          ...
          {RatingStar &&
            RatingStar.map((elem, index) => (
              <li key={elem} onClick={() => handleRate(index + 1)}>
                {index + 1 <= rating ? <RiStarFill color="#0295db" size="17" /> : <RiStarLine size="17" />}
              </li>
            ))}
        </div>
      ) : (
        <div className={`rating ${classes?.customClass ? classes?.customClass : ""}`}>{RatingStar && RatingStar.map((elem) => <span key={elem}>{elem + 1 <= totalRating ? <RiStarFill color="#0295db" size="17" /> : <RiStarLine size="17" />}</span>)}</div>
      )}
    </>
  );
};
