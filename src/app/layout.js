...
export default async function RootLayout({ children }) {
  // fetch settings and language in parallel to avoid blocking TTFB
  const [settingsRes, lng] = await Promise.all([
    fetch(`${process.env.API_PROD_URL}/settings`)
      .then((res) => res.json())
      .catch((err) => ({ error: err })),
    detectLanguage().catch(() => "en"),
  ]);
